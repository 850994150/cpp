...
* `make` 传递参数给 `makefile`  
  ...
  * make:
    ```
    make CFLAG=-DDEBUG // 给makefile传递参数
    ```
* makefile传递参数到代码: `-D DEBUG`
...
* main函数结束后
  ...
  * 用atexit注册的函数也会在main之后执行(return 0, 就是退出信号)
...
* 利用完全二叉树的结构来维护一组数据
* 堆排序
  当新增加一个数被放置到堆顶时, 如果此时不符合最小堆的特性,则将需要将这个数向下调整(与左右儿子作比较, 与小儿子做交换)直到找到合适的位置为止,使其重新符合最小堆的特性。
