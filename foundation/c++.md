...
- [C++语言基础](#c语言基础)
    ...
    - [`this`指针调用成员函数时的压栈和相关寄存器使用？](#this指针调用成员函数时的压栈和相关寄存器使用)
    - [函数符号生成规则](#函数符号生成规则)
    - [位操作](#位操作)
    ...
    - [`const` 和`#define`的区别](#const-和define的区别)
...
> `epoll` 不会随着FD的增长而降低效率(`select`存放在数组，`epoll`存放在队列)   
> `epoll` 最大的优点就在于它只管你“活跃”的连接，而跟连接总数无关

* `select` 的句柄数目受限，`linux`系统中最多同时监听1024个fd。  
  `epoll` 没有，它的限制是最大的打开文件句柄数目。

* `epoll` 的最大好处是不会随着FD的数目增长而降低效率   
  在`selec中采用轮询文件描述符的方式处理`，其中的数据结构类似一个数组的数据结构，而`epoll是维护一个队列`，直接看队列是不是空就可以了

* 使用`mmap加速内核与用户空间的消息传递`。epoll是通过内核于用户空间`mmap`同一块内存实现的，加快了消息拷贝到用户空间的过程。
...
> `epoll` 两种触发模式，`LT`是垂直触发，`ET`是边缘触发  
...
* `ET`模式中，**只会提示一次**，直到下次再有数据流入之前都不会再提示了，无论`fd`中是否还有数据可读，也就是说一直读到read的返回值小于请求值，或者 遇到EAGAIN错误


## 为什么浮点数有误差

> 二进制的小数无法精确的表达十进制小数，计算机在计算十进制小数的过程中要先转换为二进制进行计算，这个过程中出现了误差。

* 十进制小数是怎么转化为二进制的(计算机组成原理的知识)   
  整数部分采用`连除取余`，小数部分采用`连乘取整`的方法，这里连除是除以2，连乘是乘以2
  ```
  例如：将十进制数值`25.65`转换为二进制数值，步骤如下：
  25(整数部分)
  25/2=12......1
  12/2=6 ......0
  6/2=3  ......0
  3/2=1  ......1
  1/2=0  ......1
  (25) 10=(11001) 2 // 10进制25用2进制表示为11001
  ...
  (25.65) 10=(11001) 2+(0.101001...) 2=(11001.101001...) 2
  ```
* `IEEE`标准   
...
* 可以通过重载 `==` 来实现，或者写一个成员函数
  ...
  {
      return ((a == rhs.a) && (b == rhs.b)); // 这里并没有 this.a
  }
  ```
...
> [__cdecl,__stdcall,__fastcall,__pascal,__thiscall 的区别](https://www.cnblogs.com/john-h/p/6276828.html)   
> `standard call` c++标准调用方式 & `c declaration` c和c++默认的函数调用方式   
> 函数的调用约定，`描述了函数参数是怎么传递和由谁清除堆栈的`
...
> 函数体 this指针默认通过`ECX寄存器`传递给调用者，其它参数`从右到左入栈`，是C++`类成员函数缺省的调用约定`
...
> 它用来解决变量在“共享”环境下容易出现读取错误的问题，每次使用都重新从内存取数据。
...
* 编译器为`每个【类】维护一个虚函数表`(本质是一个函数指针数组,数组里面存放着该类所有虚函数的地址);  
...
> 虚函数要有_vptr指针才能访问，虚函数指针存放在对象中，是在类构造之后才有的
...
> 不可以，参数默认传递方式是拷贝，所以调用拷贝构造的时候又去找拷贝构造函数，陷入循环
...
* 拷贝构造函数 和 赋值构造函数   
  ...
  ```
* 哪些自动生成的函数要禁止? 为什么?   
  因为默认生成的这两个函数进行拷贝的时候都是进行浅拷贝, 如果类的成员中有指针的话，浅拷贝方式的结果是只拷贝了变量, 没有重新分配内存, 导致两个不同对象的指针指向同一块内存区域，容易出现访问冲突，比如多次delete等错误
...
* 就绪状态  
  参与调度，等待被执行（即其他资源已获取，就差CPU了），一旦被调度选中，立即开始执行  

* 运行状态  
  获得CPU，正在运行中  

* 阻塞状态   
  因为某种原因放弃CPU使用权，暂时停止运行。直到线程进入就绪状态，才有机会转到运行状态。
...
> [Linux下的进程信号处理过程](https://blog.csdn.net/h___q/article/details/84245317)  
...
> `TCP` 提供的是【面向连接的可靠的】传输服务，在传输数据之前会先发起三次握手建立连接，而且有【超时重传、流量控制、拥塞避免】等机制来保证传输的可靠性，因此TCP的报文头20字节比UDP的报文头8字节大，所以TCP的【传输速度】没有UDP那么快。
