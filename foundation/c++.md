...
问题的引出   
  当需要读两个以上的I/O的时候（`比如socket编程中有多个客户端连接进来`），如果使用阻塞式的I/O，那么可能长时间的阻塞在一个描述符上面，另外的描述符虽然有数据但是不能读出来，这样实时性不能满足要求，大概的解决方案有以下几种：
...
> `LT`水平触发(条件触发): socket可写就会一直提示socket可写   
> `ET`边缘触发: socket可写就提醒一次啊，等到再次可写才再次提醒  
...
* `printf` 是C语言函数，哪来的重载，c语言是通过变长参数来实现的

* 为什么格式化字符串没有也能输出字符串指针?
   我推测，因为printf输出的时候是根据格式控制字符串从栈中取字节的，比如%d就取4个字节，如果没有这个格式控制字符串，猜测是把所有都取出来，如果是字符串，因为有\0，所以可以输出 
...
> 有个变量允许在`const成员函数内改变它的值`，则用 `mutable` 修饰
