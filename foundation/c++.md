...
* 就像动物类，它是一个类别的抽象(`抽象类`)，没有实际意义，就像马、牛这样实实在在的生物，他们属于动物这个类别。这些动物都有一个属性就是[会动]，这个属性就是动物这个类的纯虚函数，但是不同的动物动的方式不一样，所以这个属性应该由马、牛这些子类去做具体的实现
...
* `operator` 不仅仅用于操作符重载，还可用于强制类型转换， 即该类型可以自动转换为char类型。
```c++
class A{
   ...
...
* 而断开连接的时候,一个方向的断开,只说明该方向数据已传输完毕 ,而另一个方向或许还有数据要发给对方,所以得等到另一个方向数据也全部传输完成后,才能执行第三次挥手
...
* 在 TCP 报文段首部的窗口字段写入的数值就是当前`给对方设置的发送窗口数值的上限`。发送窗口在连接建立时由双方商定。但在通信的过程中，接收端可根据自己的资源情况，随时动态地调整对方的发送窗口上限值(可增大或减小)。
...
> TCP 要求接收方必须有累积确认的功能，这样可以减小传输开销（累积确认：一般地讲，如果发送方发了包1，包2，包3，包4；接受方成功收到包1，包2，包3。那么接受方可以发回一个确认包，序号为4(4表示期望下一个收到的包的序号；当然你约定好用3表示也可以)，那么发送方就知道包1到包3都发送接收成功，必要时重发包4。一个确认包确认了累积到某一序号的所有包。而不是对没个序号都发确认包。）  
...
* 由发送方和接收方在三次握手阶段，`互相将自己的最大可接收的数据量告诉对方`, 也就是自己的数据接收缓冲池的大小;
   这样对方可以根据已发送的数据量来计算是否可以接着发送。在处理过程中，当接收缓冲池的大小发生变化时，要给对方发送更新窗口大小的通知
